...
export const ProductDetail: React.FC<ProductDetailProps> = ({ 
  ...
  // Simulation of more images if not provided
  ...
  const buyNowPrice = product.type === 'auction' 
        ...
        <div className="flex items-center gap-3">
          ...
          <button 
            ...
            className={`p-3 rounded-full transition-all shadow-sm border ${
              isWishlisted ? 'bg-orange-50 border-orange-200 text-orange-600' : 'bg-white border-gray-100 text-gray-400 hover:text-orange-600'
            }`}
          >
            ...
            {images.map((img, idx) => (
              <button 
                ...
                className={`aspect-square rounded-2xl overflow-hidden border-2 transition-all ${
                  activeImageIdx === idx ? 'border-orange-500 ring-4 ring-orange-500/10' : 'border-gray-100 hover:border-gray-200'
                }`}
              >
          ...
          <div className="bg-white rounded-[2.5rem] border border-gray-100 p-8 space-y-8">
            <div>
              ...
              <p className="text-gray-600 leading-relaxed">
                Ce {product.name} a été soigneusement inspecté par nos techniciens. {product.tested ? "Toutes les fonctionnalités ont été testées et sont 100% opérationnelles." : "Produit vendu en l'état."}
              </p>
          ...
          <div className="bg-white rounded-[2.5rem] border border-gray-100 shadow-xl shadow-gray-200/40 overflow-hidden">
            <div className="p-8">
              <div className="flex items-center gap-2 mb-4">
                <span className="text-[10px] font-black uppercase text-gray-400 tracking-wider">{product.category}</span>
                <span className="text-gray-200">•</span>
                <span className="text-[10px] font-black uppercase text-orange-600 bg-orange-50 px-2 py-0.5 rounded-md">Réf: SM-{product.id}</span>
              </div>
              ...
              {product.type === 'auction' ? (
                <div className="space-y-8">
                  ...
                  <div className="bg-orange-50 rounded-3xl p-6 border border-orange-100">
                    <div className="flex justify-between items-start mb-4">
                      <div>
                        <p className="text-[11px] font-black uppercase text-orange-400 mb-1">Enchère actuelle</p>
                    ...
                    <form onSubmit={handleBidSubmit} className="space-y-4">
                      <div className="flex gap-2">
                        ...
                        >
                          {isPlacingBid ? <div className="w-5 h-5 border-2 border-white/20 border-t-white rounded-full animate-spin" /> : "Enchérir"}
                        </button>
                      </div>
                      <p className="text-[10px] text-gray-400 text-center font-bold uppercase">Mise minimale : {(product.currentBid || product.price) + 1}€</p>
                    </form>
                  </div>

                  {/* Buy Now Option (Hybrid Feature) */}
                  ...
                  <div className="bg-blue-600 rounded-3xl p-6 text-white relative overflow-hidden group shadow-xl shadow-blue-600/20">
                     ...
                     <div className="relative z-10 flex flex-col sm:flex-row items-center justify-between gap-4 text-center sm:text-left">
                        <div>
                          ...
                          <p className="text-3xl font-black mt-1 text-orange-400">{buyNowPrice.toLocaleString('fr-FR')}€</p>
                        </div>
                        <button 
                ...
                <div className="space-y-6">
                  ...
                  <div className="flex items-baseline gap-3">
                    <span className="text-4xl font-black text-gray-900">{product.price.toLocaleString('fr-FR')}€</span>
                    {product.originalPrice && (
                      <span className="text-xl text-gray-400 line-through font-bold">{product.originalPrice.toLocaleString('fr-FR')}€</span>
                    )}
                    {product.originalPrice && (
                       ...
                       <div className="space-y-3">
                          {[
                            ...
                            <div key={i} className="flex items-center justify-between p-3 bg-white rounded-xl border border-gray-100">
                               <div className="flex items-center gap-3">
                                  <div className={`w-2 h-2 rounded-full ${bid.status === 'leader' ? 'bg-green-500 animate-pulse' : 'bg-gray-300'}`} />
          ...
          <div className="bg-gray-900 rounded-[2.5rem] p-8 text-white relative overflow-hidden">
             ...
             <div className="relative z-10 flex items-center gap-6">
                ...
                <div>
                   ...
                   <p className="text-sm text-gray-400">Paiement 100% sécurisé et garantie contre la casse pendant le transport.</p>
                </div>
             </div>
          </div>
