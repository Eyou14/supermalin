...
import { LayoutDashboard, Package, Inbox, Trash2, Edit3, CheckCircle, ExternalLink, RefreshCcw, Handshake, AlertCircle, Plus, X, ShieldCheck, Gavel, Tag, Users, CreditCard, Wallet, Search, Mail, Phone, Calendar, Download, Eye } from 'lucide-react';
        ...
        <nav className="flex-1 p-4 space-y-2">
          {[
            ...
            <button 
              ...
              className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl font-bold transition-all ${activeTab === tab.id ? 'bg-orange-600 text-white' : 'text-gray-400 hover:text-white hover:bg-white/5'}`}
            >
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <header className="h-20 bg-white border-b border-gray-100 flex items-center justify-between px-8 shrink-0">
          ...
          <div className="flex items-center gap-4">
            ...
            {activeTab === 'inventory' && (
              <button onClick={() => setIsAddModalOpen(true)} className="bg-orange-600 text-white px-5 py-2.5 rounded-xl text-sm font-bold flex items-center gap-2">
                <Plus size={18} /> Article
              </button>
            )}
            {activeTab === 'users' && (
              <button onClick={() => setIsUserModalOpen(true)} className="bg-orange-600 text-white px-5 py-2.5 rounded-xl text-sm font-bold flex items-center gap-2">
        ...
        <main className="flex-1 overflow-y-auto p-8">
          {isLoading ? (
                    ...
                    <tr key={order.id} className="hover:bg-gray-50/50 transition-all">
                      ...
                      <td className="px-6 py-4">
                        <div className="flex flex-col gap-1">
                          {order.items.map((item: any, idx: number) => (
                            <span key={idx} className="text-xs text-gray-600">• {item.name}</span>
                          ))}
                        </div>
                      </td>
                      <td className="px-6 py-4">
                      ...
                      <td className="px-6 py-4">
                        <span className={`px-2 py-0.5 rounded-full text-[10px] font-black uppercase ${
                          ...
                          {order.status === 'paid' ? 'Payé' : order.status === 'pending' ? 'Attente' : order.status}
                        </span>
                      </td>
                      <td className="px-6 py-4 text-right">
                        <div className="flex items-center justify-end gap-2">
                           ...
                           {order.status === 'paid' && (
                             <button className="p-2 text-gray-400 hover:text-green-600"><CheckCircle size={18} /></button>
                           )}
                        </div>
                      </td>
                    </tr>
                    ...
                    <tr key={user.userId} className="hover:bg-gray-50/50 transition-all group">
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          <img src={user.avatar || `https://i.pravatar.cc/150?u=${user.userId}`} className="w-10 h-10 rounded-full object-cover" />
                ...
                <tbody className="divide-y divide-gray-50">
                  {transactions.map((tr) => (
                    <tr key={tr.id} className="hover:bg-gray-50/50 transition-all">
                      ...
                      <td className="px-6 py-4">
                        <span className="px-2 py-0.5 bg-green-100 text-green-700 rounded-full text-[10px] font-black uppercase">{tr.status}</span>
                      </td>
                      <td className="px-6 py-4 text-right">
            ...
            <div className="space-y-4">
              {requests.map((req) => (
                <div key={req.id} className="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm flex items-center justify-between hover:shadow-md transition-all">
                  <div className="flex items-center gap-6">
                    <div className={`w-14 h-14 rounded-2xl flex items-center justify-center ${req.type === 'buyback' ? 'bg-orange-50 text-orange-600' : 'bg-blue-50 text-blue-600'}`}>
                  ...
                  <div className="flex items-center gap-4">
                    <select 
                      ...
                      className={`px-4 py-2 rounded-xl text-xs font-bold border-none outline-none cursor-pointer ${requestStatuses.find(s => s.id === (req.status || 'pending'))?.color}`}
                    >
                ...
                <tbody className="divide-y divide-gray-50">
                  {inventory.filter(i => i?.name?.toLowerCase().includes(searchTerm.toLowerCase())).map((item) => (
                    <tr key={item.id} className="hover:bg-gray-50/50 transition-all group">
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          <img src={item.image} className="w-10 h-10 rounded-xl object-cover bg-gray-100" />
                          <span className="font-bold text-gray-900">{item.name}</span>
                        </div>
                      </td>
                      <td className="px-6 py-4 text-xs font-bold uppercase">{item.type === 'auction' ? 'Enchère' : 'Direct'}</td>
                      <td className="px-6 py-4 font-black">{(item.type === 'auction' ? item.currentBid : item.price)?.toLocaleString()}€</td>
                      <td className="px-6 py-4 text-right">
                        <button onClick={() => deleteProduct(item.id)} className="p-2 text-gray-400 hover:text-red-600 transition-all"><Trash2 size={18} /></button>
                      </td>
                    </tr>
                  ))}
                </tbody>
      ...
      <AnimatePresence>
        {isAddModalOpen && (
          <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="relative bg-white w-full max-w-lg rounded-[2.5rem] p-10 shadow-2xl overflow-y-auto max-h-[90vh]">
              ...
              <form onSubmit={handleAddProduct} className="grid grid-cols-2 gap-4">
                <div className="col-span-2 space-y-1">
                  ...
                  <input name="name" required className="w-full bg-gray-50 border border-gray-100 rounded-xl px-4 py-3 text-sm outline-none" />
                </div>
                <div className="space-y-1">
                  <label className="text-[10px] font-black uppercase text-gray-400">Prix (€)</label>
                  <input name="price" type="number" step="0.01" required className="w-full bg-gray-50 border border-gray-100 rounded-xl px-4 py-3 text-sm outline-none" />
                </div>
                <div className="space-y-1">
                  <label className="text-[10px] font-black uppercase text-gray-400">Stock</label>
                  <input name="stock" type="number" defaultValue="1" className="w-full bg-gray-50 border border-gray-100 rounded-xl px-4 py-3 text-sm outline-none" />
                </div>
                <div className="col-span-2 space-y-1">
                  <label className="text-[10px] font-black uppercase text-gray-400">URL Image</label>
                  <input name="image" required className="w-full bg-gray-50 border border-gray-100 rounded-xl px-4 py-3 text-sm outline-none" />
                </div>
                <div className="space-y-1">
                  <label className="text-[10px] font-black uppercase text-gray-400">Catégorie</label>
                  <select name="category" className="w-full bg-gray-50 border border-gray-100 rounded-xl px-4 py-3 text-sm outline-none">
                ...
                <div className="space-y-1">
                  ...
                  <select name="type" className="w-full bg-gray-50 border border-gray-100 rounded-xl px-4 py-3 text-sm outline-none">
                    <option value="direct">Achat immédiat</option>
                    <option value="auction">Enchères</option>
                  </select>
                </div>
                <div className="col-span-2 space-y-1">
                  <label className="text-[10px] font-black uppercase text-gray-400">Description</label>
                  <textarea name="description" rows={3} className="w-full bg-gray-50 border border-gray-100 rounded-xl px-4 py-3 text-sm outline-none resize-none" />
                </div>
                <button type="submit" className="col-span-2 bg-orange-600 text-white py-4 rounded-xl font-bold mt-4 hover:bg-orange-700 transition-all">Publier l'article</button>
              </form>
            </motion.div>
          </div>
        )}
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="relative bg-white w-full max-w-md rounded-[2.5rem] p-10 shadow-2xl">
              ...
              <form onSubmit={handleCreateUser} className="space-y-4">
                ...
                <div className="space-y-1">
                  ...
                  <input name="email" type="email" required className="w-full bg-gray-50 border border-gray-100 rounded-xl px-4 py-3 text-sm outline-none" />
                </div>
                <div className="space-y-1">
                  <label className="text-[10px] font-black uppercase text-gray-400">Mot de passe</label>
                  <input name="password" type="password" required className="w-full bg-gray-50 border border-gray-100 rounded-xl px-4 py-3 text-sm outline-none" />
                </div>
                <button type="submit" className="w-full bg-orange-600 text-white py-4 rounded-xl font-bold mt-4 hover:bg-orange-700 transition-all">Créer le compte</button>
              </form>
            </motion.div>
          </div>
        )}
