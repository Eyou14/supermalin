...
import { Package, Gavel, Settings, LogOut, ChevronRight, Clock, CheckCircle2, AlertCircle, User, CreditCard, Heart, ArrowUpRight, ArrowDownLeft, Plus, History, TrendingUp, Trophy, Target, Share2, Gift, Users, Mail, Send, Link, Twitter, Facebook, MessageCircle, Handshake, Camera, MapPin, Trash2, Edit2, Check, Download, FileText, RefreshCcw } from 'lucide-react';
...
export const UserProfile = ({ 
  ...
  const [activeTab, setActiveTab] = useState<'orders' | 'settings' | 'selling'>('orders');
  ...
  const orders: Order[] = [
    {
      ...
      items: ['iPhone 13 Pro 128Go - Bleu'],
      image: 'https://images.unsplash.com/photo-1632661674596-df8be070a5c5?q=80&w=200'
    }
  ];

  const [isAddingAddress, setIsAddingAddress] = useState(false);
  const [addressForm, setAddressForm] = useState<Partial<Address>>({
    label: '', fullName: '', street: '', city: '', zipCode: ''
  });
  ...
  const handleAddAddress = () => {
    ...
    setAddressForm({ label: '', fullName: '', street: '', city: '', zipCode: '' });
        ...
        <div className="lg:w-1/4">
          <div className="bg-white rounded-3xl border border-gray-100 p-6 shadow-sm sticky top-24">
            <div className="flex flex-col items-center text-center mb-8">
              ...
              <h2 className="text-xl font-bold text-gray-900">{profile?.name || user?.user_metadata?.name || 'Utilisateur'}</h2>
              <p className="text-sm text-gray-500">Membre depuis {new Date(profile?.createdAt || user?.created_at).toLocaleDateString('fr-FR', { month: 'long', year: 'numeric' })}</p>
            ...
            <nav className="space-y-1">
              {[
                ...
                <button
                  ...
                  className={`w-full flex items-center justify-between px-4 py-3 rounded-xl text-sm font-bold transition-all ${
                    activeTab === item.id ? 'bg-gray-900 text-white shadow-lg shadow-gray-900/10' : 'text-gray-600 hover:bg-gray-50'
                  }`}
                >
              ...
              <button onClick={onLogout} className="w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-bold text-red-500 hover:bg-red-50 transition-all mt-4">
            ...
            <div className="space-y-6">
              ...
              {isLoadingOrders ? (
                ...
                <div className="bg-white rounded-3xl border border-gray-100 p-12 text-center">
                   ...
                   <button onClick={() => onNavigate('shop')} className="mt-4 text-orange-600 font-bold hover:underline">Découvrir la boutique</button>
                </div>
              ) : (
                      ...
                      <div className="flex items-center gap-6">
                        ...
                        <div>
                          <div className="flex items-center gap-2 mb-1">
                            <span className="text-[10px] font-black uppercase text-gray-400">Commande {order.id}</span>
                          ...
                          <h3 className="font-bold text-gray-900 mb-1">
                            {order.items.length} article(s) • {order.items[0]?.name}
                          </h3>
                          <p className="text-sm text-gray-500">Passée le {new Date(order.createdAt).toLocaleDateString('fr-FR')}</p>
                        </div>
                      </div>
            ...
            <div className="space-y-8">
              ...
              <div className="bg-white rounded-3xl border border-gray-100 p-8 shadow-sm">
                <div className="flex items-center justify-between mb-6">
                  ...
                  {!isEditing && (
                    <button onClick={() => setIsEditing(true)} className="text-xs font-black uppercase text-orange-600 bg-orange-50 px-3 py-1.5 rounded-lg">Modifier</button>
                  )}
                </div>
                ...
                <form className="space-y-6">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="space-y-2">
                      ...
                      <input type="email" value={editForm.email} disabled className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-sm opacity-70" />
                    </div>
                    <div className="space-y-2">
                  ...
                  {isEditing && (
                    <div className="flex items-center gap-4">
                      ...
                      <button type="button" onClick={() => setIsEditing(false)} className="text-gray-500 font-bold text-sm hover:text-gray-700">Annuler</button>
                    </div>
                  )}
                </form>
              </div>
